...
3. gixt contacts GitHub (unless `--offline`) with the cached ETag: if the gist is unchanged the server replies `304` and the cached copy is reused — no files are downloaded. With `--no-cache` files go to a temp dir instead.
4. Files are materialized into a content-addressed cache dir `cache/<id>/<sha>` with path sanitization (no `..`, no absolute or drive-prefixed paths).
5. Trust is checked (see below). On first use of a revision you are prompted; `-y` skips the prompt.
6. The command is resolved: `--python <interpreter>` override, else the shebang on the chosen file, else the extension map (`.py` -> `python`, `.sh` -> `sh`, `.js` -> `node`, `.ts` -> `npx ts-node`, `.go` -> `go run`, `.rb` -> `ruby`, `.pl` -> `perl`, `.php` -> `php`, `.ps1` -> `powershell`, `.bat/.cmd` -> `cmd /C` on Windows).
7. The command runs in your current directory (or the gist work dir with `--isolate`). Child stdin/stdout/stderr are wired straight through; the child's exit code is propagated. `--timeout` cancels long runs.
8. After a successful run, older cached revisions of that gist are pruned (only the latest is kept). With `--add` (or `--as <name>`, which implies `--add`), the gist is also remembered in `known.json`.
...
- `--no-cache`: download to a temp dir and delete it after the run (also honored via `GIXT_NO_CACHE`).
